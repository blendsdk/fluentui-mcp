...
| AR-13 | Technical (runtime) | The incremental enhancer regenerates every guide in Pass 2 even when nothing changed, which would make 38 paid LLM calls and rewrite guide prose outside provenance scope | Run the full incremental enhancer / run with `--components-only` so guides carry forward | Run `enhance --components-only`; components and guides carry forward with zero LLM calls | ✅ Resolved — Necessary correction (no behavior change) |
| AR-14 | Technical (runtime) | ST-10 needs a drift check of a temporary tree, but `checkDrift` regenerated with the repository `package.json`, so the temp version never matched | Extend `checkDrift` with an optional `packageJsonPath` / drop the drift assertion | Extend `checkDrift` with an optional `packageJsonPath` forwarded to `generateSkill` | ✅ Resolved — Necessary correction (testability) |
