/**
 * TF-IDF search engine for the schema-driven FluentUI documentation server.
 *
 * Provides full-text search across all indexed documents using a term
 * frequency–inverse document frequency (TF-IDF) scoring model.
 *
 * In the schema-driven era the engine no longer reads markdown files from
 * disk. Instead {@link module:search/search-index} synthesises
 * {@link DocumentEntry} records from the structured schema (components,
 * utilities, guides, patterns) and feeds them to this engine. The scoring
 * model, tokenizer, and result shape are unchanged, so the `search_docs`
 * tool and the search-backed intelligence tools behave identically.
 *
 * The engine tokenizes document content at index-build time, then scores
 * queries against the pre-built inverted index for sub-millisecond search.
 *
 * Features:
